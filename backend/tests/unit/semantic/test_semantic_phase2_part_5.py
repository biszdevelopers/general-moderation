...
class TestDeleteRoundtrip(BaseTest):
    ...
    @pytest.mark.parametrize(
        ...
        _DELETE_ROUNDTRIP_CASES,
    )
    def test_delete_roundtrip(
        self, settings: Any, fake_semantic_modules: None, category: str, count: int, uid: int
    ) -> None:
        """Deleting added examples restores the baseline count."""
...
class TestStatsField(BaseTest):
    ...
    def test_stats_field(
        self, settings: Any, fake_semantic_modules: None, category: str, stat_key: str, uid: int
    ) -> None:
        """Stats expose every category with a non-negative count."""
