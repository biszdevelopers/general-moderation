...
class TestVerdictExactMatrix(BaseTest):
    ...
    @pytest.mark.parametrize(
        ...
        _VERDICT_EXACT_MATRIX_CASES,
    )
    def test_verdict_exact_matrix(
        self,
...
class TestVerdictTypoMatrix(BaseTest):
    ...
    @pytest.mark.parametrize(
        ...
        _VERDICT_TYPO_MATRIX_CASES,
    )
    def test_verdict_typo_matrix(
        self,
        engine: ModerationEngine,
        word_bank: Any,
        text: str,
...
class TestCacheSizes(BaseTest):
    ...
    def test_cache_bounded_0_1_1397(self, engine: ModerationEngine) -> None:
    ...
    def test_cache_bounded_0_60_1398(self, engine: ModerationEngine) -> None:
        """The result cache never exceeds its configured size."""
        engine.moderate(ModerationRequest(text="seed cache", app_name="a"))
        assert len(engine._cache) <= 100
        assert engine._cache is not None

    def test_cache_bounded_0_300_1399(self, engine: ModerationEngine) -> None:
        """The result cache never exceeds its configured size."""
        engine.moderate(ModerationRequest(text="seed cache", app_name="a"))
        assert len(engine._cache) <= 100
        assert engine._cache is not None

    def test_cache_bounded_1_1_1400(self, engine: ModerationEngine) -> None:
        """The result cache never exceeds its configured size."""
        engine.moderate(ModerationRequest(text="seed cache", app_name="a"))
        assert len(engine._cache) <= 100
        assert engine._cache is not None
